...
@section('content')
<style>
    .search-input-width { width: 300px; }
    .user-row { cursor: pointer; transition: background 0.2s; }
    .user-row:hover { background-color: rgba(105, 108, 255, 0.05) !important; }
        ...
        @foreach($usuarios as $usuario)
          ...
          <tr class="user-row">
            <td onclick="abrirModalUsuario({{ json_encode($usuario) }})">
              <div class="d-flex align-items-center">
                <div class="avatar avatar-sm {{ $color }} me-3 d-flex align-items-center justify-content-center rounded-circle">
          ...
          <td class="text-center" onclick="event.stopPropagation()">
            <div class="d-flex justify-content-center gap-2">
              <button type="button" class="btn btn-sm btn-primary" onclick="abrirModalUsuario({{ json_encode($usuario) }})">
              ...
              <form action="{{ route('usuarios.destroy', $usuario->id) }}" method="POST" class="d-inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('¿Seguro que deseas eliminar definitivamente a este usuario?')">
            ...
            <div class="col-md-12 mt-4">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group input-group-merge">
                        ...
                        <span class="input-group-text cursor-pointer" onclick="togglePassword('edit_password_confirmation', 'toggleIcon2')"><i class="ri-eye-line" id="toggleIcon2"></i></span>
                    </div>
                    <div id="password-match-msg" class="form-text mt-1 d-none text-danger">Las contraseñas no coinciden.</div>
                </div>
            </div>
          </div>
        </div>
